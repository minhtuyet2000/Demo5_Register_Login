...
public class WebUI {
    ...
    public static void waitForElementVisible(By by, int index, int TIMEOUT) {
        try {
            WebElement element = getWebElements(by).get(index);
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(TIMEOUT));
            wait.until(ExpectedConditions.visibilityOf(element));
        } catch (Throwable error) {
            logConsole("Hết thời gian chờ hiển thị. Element - " + by + ".get(" + index + ")");
        }
    }
    ...
    public static void scrollToClick(int x, int y) {
        JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();
    ...
    public static void clearWithAction(By by) {
        waitForPageLoaded();
        waitForElementVisible(by);
        sleep(STEP_TIME);
        WebElement element = getWebElement(by);
        ...
        logConsole("Clear element by action-" + element.getText() + " " + by);
        element.click();
    ...
    public static void clickElement(By by, int index) {
        ...
        logConsole("Click element -" + element.getText() + " " + by + ".get(" + index + ")");
        try {
            JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();
            js.executeScript("arguments[0].scrollIntoView(true);", element);
            element.click();
        } catch (Exception e) {
            logConsole("Click element by action-" + element.getText() + " " + by + ".get(" + index + ")");
            try {
                Actions actions = new Actions(DriverManager.getDriver());
                actions.moveToElement(element).click().build().perform();
            } catch (Exception ex) {
                logConsole("Click element by js-" + element.getText() + " " + by + ".get(" + index + ")");
                try {
                    JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();
                    js.executeScript("arguments[0].click();", element);
                } catch (Exception ejs) {
                    logConsole("❌ Không thể click. Element - " + by + ".get(" + index + ")");
                    String log = ejs.getMessage();
                    if (log.length() > 300) {
                        log = log.substring(0, 300) + "...";
                    }
    ...
    public static String getAttributeText(By by, int index, String attribute) {
        waitForPageLoaded();
        waitForElementVisible(by);
        sleep(STEP_TIME);
        List<WebElement> elements = getWebElements(by);
    ...
    public static void assertContains(String actual, String expected, String messagePass) {
        try {
            boolean assertContains = actual.trim().contains(expected.trim());
            if (assertContains) {
                logConsole("✅ Assert Passed: " + messagePass);
            } else {
                logConsole("❌ Assert Failed - Expected: " + expected + " - Actual: " + actual);
            }
            softAssert.get().assertTrue(assertContains);
        } catch (Exception e) {
            logConsole("❌ Assert Failed - Message Pass: " + messagePass);
            logConsole("Lý do: " + e.getMessage());
        }
    }
    public static void assertNotContains(String actual, String notContains, String messagePass) {
        try {
            boolean assertContains = actual.trim().contains(notContains.trim());
            if (!assertContains) {
                logConsole("✅ Assert Passed: " + messagePass);
            } else {
                logConsole("❌ Assert Failed - Contains: " + notContains);
            }
            softAssert.get().assertFalse(assertContains);
        } catch (Exception e) {
            logConsole("❌ Assert Failed - Message Pass: " + messagePass);
            logConsole("Lý do: " + e.getMessage());
        }
    }
    public static void assertBoolean(boolean actual) {
    ...
    public static int sizeActualTexts() {
        return actualTexts.get().size();
    }

    public static String getActualText(int index) {
        return actualTexts.get().get(index);
    }
    ...
    public static void assertDisplayForAttribute(By by, int index, String attribute) {
        boolean actual = false;
        try {
            waitForPageLoaded();
            sleep(1);
